...
export default function CarDetailPage({ params }: { params: { cid: string } }) {
  ...
  async function handleCreateBooking(startDate:string,endDate:string){
    if(!session) return
    const res = await createBooking(session.user.token,params.cid,session.user.User_info._id,startDate,endDate)
    if(res.success){
  ...
  return (
    <main className="text-center p-8 min-h-screen flex flex-col items-center font-[Verdana,Geneva,Tahoma,sans-serif]">
      ...
      <button onClick={()=>{handleCreateBooking(dayjs(startDate).format("YYYY-MM-DDTHH:mm:ss[+00:00]").toString(),dayjs(endDate).format("YYYY-MM-DDTHH:mm:ss[+00:00]").toString())}}
