...
export default async function CarCatalog({ carJson }: { carJson: CarJson }) {
  ...
  return (
    <>
      {/* Header */}
      <div className="w-full max-w-screen-xl mx-auto px-4 mb-6 flex justify-between items-center">
        <h2 className="text-xl font-bold tracking-widest">
          PICK YOUR CAR FOR NEW EXPERIENCE
        </h2>
        <Link
          href="/car"
          className="border border-black px-4 py-1 rounded-full text-sm hover:bg-black hover:text-white transition"
        >
          View all
        </Link>
      </div>
      ...
      {/* Card Layout */}
      <div className="w-full min-h-screen flex flex-wrap justify-center gap-8 p-4 overflow-x-hidden">
