...
const redeemCoupon = async (couponName: string, percentage: number, maxDisc: number, minSp: number, valid: number, redeemed: boolean) => {
  ...
  const response = await createCoupon(token, couponName, percentage, maxDisc, minSp, new Date(Date.now() + valid * 24 * 60 * 60 * 1000));
  redeemed = true;
  console.log(response);
  alert("Coupon redeemed successfully!");
}

export default function CouponCard( {couponName, percentage, maxDisc, minSp, spent, valid, redeemed}
  : {couponName:string, percentage: number, maxDisc: number, minSp: number, spent: number, valid: number, redeemed: boolean} ) {

  return (
    <div>
    {
      redeemed ? (
        <div className="w-[230px] h-[333px] rounded-[24px] bg-gray-400 text-white overflow-hidden shadow-lg
        relative m-10">  
          {/* ครึ่งวงกลมด้านบน */}
          ...
          <div className="px-6 mt-3 pb-2 text-[11px] space-y-3 text-white text-left leading-[12px]">
            ...
            <p>
              Maximum Discount: <span className="font-bold text-[15px] font-rockwellCondensed">${maxDisc}</span>
            </p>
            <p>
            ...
            <div className="text-center">
              <button className="px-4 py-2 mt-1 bg-white text-gray-400 rounded shadow font-semibold"
              disabled>
                REDEEM
              </button>
            </div>
        ...
        relative m-10 hover:scale-105 transition-transform duration-300">  
          ...
          <div className="px-6 mt-3 pb-2 text-[11px] space-y-3 text-white text-left leading-[12px]">
            ...
            <div className="text-center">
              <button className="px-4 py-2 mt-1 bg-white text-black rounded shadow font-semibold
              hover:scale-105 transition-transform duration-300"
                onClick = {() => {
                redeemCoupon(couponName, percentage, maxDisc, minSp, valid, redeemed);
              }}>
                REDEEM
              </button>
            </div>
