  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6 px-4">
      {couponJson.data.map((coupon) => {
        ...
        const formattedDate = new Date(coupon.expirationDate).toLocaleDateString('en-GB');

        const textColor = isExpired ? "text-red-600" : "text-black";
        const borderColor = isExpired ? "border-red-600 hover:bg-red-100" : "border-black hover:bg-gray-100";
